...
export class OrderService {
    async createOrderFromCart(
        ...
        try {
            ...
            return await Order.findByPk(order.id, { include: [OrderItem] }) as Order;
        } catch (error) {
    ...
    async getAllOrders(filters?: OrderFilterInput): Promise<{ orders: Order[]; totalCount: number }> {
        const where: any = {};
        ...
        const { rows: orders, count: totalCount } = await Order.findAndCountAll(findOptions);
